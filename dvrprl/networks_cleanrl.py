"""
This is fundamentally the same as the networks.py script except that the networks in policy mode return
...
from dvrprl.layers import ParallelEmbeddingLayer, layer_init
...
class ParallelMultilayerPerceptron(torch.nn.Module):
    ...
    def __init__(
        self, in_features: int, embedding_dim: int, hidden_layers: list[int], std: float
    ) -> None:
        ...
        self.final_layer = layer_init(torch.nn.Linear(in_features=embedding_dim, out_features=1), std = std)
